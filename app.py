    ...
    if hd_id and partner_name:
        ...
        if file_name:
            conn.execute('''
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            ''', (hd_id, partner_name, int(volume or 0), float(discount or 0.0), start_date, end_date, file_name, status))
        else:
            conn.execute('''
                INSERT OR REPLACE INTO contracts (id, partner_name, volume, discount, start_date, end_date, status)
                VALUES (?, ?, ?, ?, ?, ?, ?)
            ''', (hd_id, partner_name, int(volume or 0), float(discount or 0.0), start_date, end_date, status))
        conn.commit()
        conn.close()
    return redirect(url_for('admin_panel'))

@app.route('/download/contract/<filename>')
...
@app.route('/api/crm/delete/<patient_id>', methods=['POST'])
def delete_crm(patient_id):
    conn = get_db_connection()
    conn.execute("DELETE FROM crm_patients WHERE id = ?", (patient_id,))
    conn.commit()
    conn.close()
    return redirect(url_for('admin_panel'))

@app.route('/api/iot/book', methods=['POST'])
def iot_book_driver():
    driver_name = request.form.get('driver_name')
    driver_phone = request.form.get('driver_phone')
    box_code = request.form.get('box_code')
    if driver_name and box_code:
        trip_id = f"TRIP_{datetime.now().strftime('%M%S')}"
        raw_msg = f"[ĐIỀU PHỐI ĐƠN LẺ]\nTài xế: {driver_name}\nThùng IoT: {box_code}\nVui lòng chạy lệnh điều phối."
        encoded_msg = urllib.parse.quote(raw_msg)
        conn = get_db_connection()
        conn.execute('INSERT INTO iot_logistics (trip_id, driver_name, driver_phone, box_code, status, zalo_msg) VALUES (?, ?, ?, ?, ?, ?)', 
                     (trip_id, driver_name, driver_phone, box_code, 'Đã điều phối xe', encoded_msg))
        conn.commit()
        conn.close()
    return redirect(url_for('admin_panel'))

@app.route('/api/iot/delete/<trip_id>', methods=['POST'])
def delete_iot(trip_id):
    conn = get_db_connection()
    conn.execute("DELETE FROM iot_logistics WHERE trip_id = ?", (trip_id,))
    conn.commit()
    conn.close()
    return redirect(url_for('admin_panel'))
...
def add_single_test():
    ...
    if name:
        ...
        ''', (name, request.form.get('type'), request.form.get('tube_type'), request.form.get('storage'),
              int(request.form.get('price', 0) or 0), request.form.get('function'), request.form.get('eta'), request.form.get('lab')))
        conn.commit()
        conn.close()
    return redirect(url_for('admin_panel'))

@app.route('/api/tests/upload-file', methods=['POST'])
def upload_bulk_tests_file():
        ...
        for row in csv_input:
            if len(row) >= 8:
                conn.execute('''
                    INSERT OR REPLACE INTO tests_catalog (name, type, tube_type, storage, price, function, eta, lab)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                ''', (row[0], row[1], row[2], row[3], int(row[4] or 0), row[5], row[6], row[7]))
        conn.commit()
        conn.close()
        return jsonify({"status": "success", "message": "Nạp file thành công!"})
    except Exception as e: 
        return jsonify({"status": "error", "message": str(e)}), 500

@app.route('/api/tests/delete/<name>', methods=['POST'])
def delete_test(name):
    conn = get_db_connection()
    conn.execute("DELETE FROM tests_catalog WHERE name = ?", (name,))
    conn.commit()
    conn.close()
    return redirect(url_for('admin_panel'))
